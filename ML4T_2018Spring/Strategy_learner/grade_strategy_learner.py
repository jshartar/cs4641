...
# from StrategyLearner imoprt StrategyLearner
import numpy as np
import pandas as pd
...
def test_strategy(description, insample_args, outsample_args, benchmark_type, benchmark, impact, train_time, test_time, max_time, seed, grader = None):
    ...
    try:
        ...
        msgs = []
        in_trades_1, in_trades_2, out_trades, train_t, test_t, out_test_t = timeoutwrapper_strategylearner()
        incorrect = False
        ...
        # tb_list = [row for row in tb_list if row[0] in ['QLearner.py','StrategyLearnerQ.py']]
        if tb_list:
            msg += "Traceback:\n"
